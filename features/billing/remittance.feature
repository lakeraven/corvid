...
Feature: Clearinghouse Remittance Processing (835)
  ...
  Scenario: Match remittance to existing claim
    Given a claim exists with Clearinghouse ID "CLM-MATCH-001"
  ...
  Scenario: Match remittance with patient responsibility
    Given a claim exists with Clearinghouse ID "CLM-PATIENT-001" and billed amount "$200.00"
    And the claim has status "accepted"
  ...
  Scenario: Handle unmatched claim in remittance
    Given no claim exists with Clearinghouse ID "CLM-UNKNOWN-001"
  ...
  Scenario: Handle duplicate remittance processing
    Given a claim exists with Clearinghouse ID "CLM-DUP-001"
  ...
  Scenario: Process multiple claims in one remittance
    Given these claims exist:
      | clearinghouse_id       | status   | billed_amount |
  ...
  Scenario: Polling job handles API errors gracefully
    Given the Clearinghouse API returns an error
    When the remittance polling job runs
  ...
  Scenario: Handle claim denial in remittance
    Given a claim exists with Clearinghouse ID "CLM-DENIED-001"
    And the claim has status "accepted"
  ...
  Scenario: Handle partial denial with adjustment codes
    Given a claim exists with Clearinghouse ID "CLM-PARTIAL-001" and billed amount "$500.00"
    And the claim has status "accepted"
  ...
  Scenario: Remittance processing is logged
    Given a claim exists with Clearinghouse ID "CLM-AUDIT-001"
  ...
  Scenario: View remittance history for a claim
    Given a claim exists with Clearinghouse ID "CLM-HISTORY-001"
  ...
  Scenario: Calculate payment statistics
    Given these paid claims exist:
      | clearinghouse_id  | billed_amount | paid_amount |
  ...
  Scenario: Handle timeout when fetching remittances
    Given the Clearinghouse API times out
