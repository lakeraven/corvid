...
Feature: Clearinghouse Claim Status Tracking
  ...
  Scenario: Check status of a submitted claim
    Given a claim submission exists with Clearinghouse ID "CLM-2025-001"
    And the claim has status "submitted"
    When I check the status of the claim
    Then I should see the current status from Clearinghouse
    And the claim status should be updated

  Scenario: Claim accepted by payer
    Given a claim submission exists with Clearinghouse ID "CLM-2025-002"
    And the claim has status "submitted"
    And Clearinghouse reports the claim is "accepted"
    When I check the status of the claim
    Then the claim status should be "accepted"
    And I should see the acceptance date

  Scenario: Claim paid by payer
    Given a claim submission exists with Clearinghouse ID "CLM-2025-003"
    And the claim has status "accepted"
    And Clearinghouse reports the claim is "paid" with amount "$120.00"
    When I check the status of the claim
    Then the claim status should be "paid"
    And the paid amount should be "$120.00"

  Scenario: Claim rejected by payer
    Given a claim submission exists with Clearinghouse ID "CLM-2025-004"
    And the claim has status "submitted"
    And Clearinghouse reports the claim is "rejected" with reason "Invalid member ID"
    When I check the status of the claim
  ...
  Scenario: Claim pending with payer
    Given a claim submission exists with Clearinghouse ID "CLM-2025-005"
    And the claim has status "submitted"
    And Clearinghouse reports the claim is "pending"
    When I check the status of the claim
  ...
  Scenario: Background job handles Clearinghouse API errors gracefully
    Given there are 3 pending claims
    And the Clearinghouse API returns an error for the second claim
    When the claim status polling job runs
  ...
  Scenario: Status changes are logged
    Given a claim submission exists with Clearinghouse ID "CLM-2025-030"
    And the claim has status "submitted"
  ...
  Scenario: Alert on claim rejection
    Given a claim submission exists with Clearinghouse ID "CLM-2025-040"
    And the claim has status "submitted"
  ...
  Scenario: Alert on claim payment
    Given a claim submission exists with Clearinghouse ID "CLM-2025-041"
  ...
  Scenario: Handle claim not found in Clearinghouse
    Given a claim submission exists with Clearinghouse ID "CLM-NOTFOUND"
    When I check the status of the claim
    Then I should see error "Claim not found"
    And the claim should be flagged for review

  Scenario: Handle Clearinghouse API timeout
    Given a claim submission exists with Clearinghouse ID "CLM-TIMEOUT"
    And the Clearinghouse API times out
    When I check the status of the claim
