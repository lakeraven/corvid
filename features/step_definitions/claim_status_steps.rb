...
# Claim status step definitions (ported from the predecessor app)
...
Given("Clearinghouse reports the claim is {string}") do |status|
  mapped = status.downcase
  Corvid.adapter.add_claim(@claim_submission.claim_identifier, { status: mapped, paid_amount: nil })
end

Given("Clearinghouse reports the claim is {string} with amount {string}") do |status, amount|
...
Given("Clearinghouse reports the claim is {string} with reason {string}") do |status, reason|
  Corvid.adapter.add_claim(@claim_submission.claim_identifier, {
    status: status.downcase,
...
Given("the following claims are pending:") do |table|
  @pending_claims = table.hashes.map do |row|
    claim = Corvid::ClaimSubmission.create!(
      tenant_identifier: @tenant,
      facility_identifier: @facility,
      patient_identifier: "pt_billing_#{row['clearinghouse_id']}",
    ...
    if row["clearinghouse_reports"]
      Corvid.adapter.add_claim(row["clearinghouse_id"], {
        status: row["clearinghouse_reports"].downcase,
