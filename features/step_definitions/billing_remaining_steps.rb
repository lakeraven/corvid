...
Given("no claim exists with Clearinghouse ID {string}") do |clearinghouse_id|
  refute Corvid::ClaimSubmission.exists?(claim_identifier: clearinghouse_id)
  Corvid.adapter.add_remittance("REM_ORPHAN_#{clearinghouse_id}", {
    remittance_identifier: "REM_ORPHAN_#{clearinghouse_id}",
    payer_name: "Test Payer", payment_date: Date.current, total_paid: 100.00,
    line_items: [ { claim_identifier: clearinghouse_id, paid_amount: 100.00 } ]
  })
end
...
Given("these claims exist:") do |table|
  @multi_claims = table.hashes.map do |row|
    Corvid::ClaimSubmission.create!(
      ...
      patient_identifier: "pt_multi_#{row['clearinghouse_id']}",
      claim_identifier: row["clearinghouse_id"],
...
Given("these paid claims exist:") do |table|
  @multi_claims = table.hashes.map do |row|
    Corvid::ClaimSubmission.create!(
      tenant_identifier: @tenant, facility_identifier: @facility,
      patient_identifier: "pt_paid_#{row['clearinghouse_id']}",
      claim_identifier: row["clearinghouse_id"],
...
Given("a remittance includes payments:") do |table|
  line_items = table.hashes.map do |row|
    claim_id = row["claim_id"] || row["clearinghouse_id"]
...
Then("claim {string} should be paid") do |clearinghouse_id|
  claim = Corvid::ClaimSubmission.find_by(claim_identifier: clearinghouse_id)
  refute_nil claim, "Expected claim #{clearinghouse_id}"
...
Given("the Clearinghouse API returns an error") do
  Corvid.adapter.define_singleton_method(:fetch_remittances) do |**_|
    raise StandardError, "Clearinghouse API error"
  end
end
