...
# Claims submission step definitions (ported from a prior implementation)
...
Given("the Clearinghouse API is unavailable") do
  Corvid.adapter.define_singleton_method(:submit_claim) do |_data|
...
When("Clearinghouse reports the claim status changed to {string}") do |new_status|
...
Then("I should see a Clearinghouse claim ID") do
  refute_nil @claim_submission.claim_identifier
end
...
Then("I should see the Clearinghouse claim ID") do
...
Then("each claim should have a unique Clearinghouse claim ID") do
